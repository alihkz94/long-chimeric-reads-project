...
# Set working directory
setwd("/gpfs/space/home/alihakim/mypc/dada2/66")
...
# Chimera removal (if not already done)
nonchimeric_seqtab <- removeBimeraDenovo(seqtab, method = "consensus", multithread = TRUE,
                                         minFoldParentOverAbundance = 1.5, minParentAbundance = 2, 
...
# Ensure that chimeric_sequences are character strings
chimeric_sequences <- as.character(chimeric_dna)

# Create headers for the FASTA file
chimera_headers <- paste0(">", "chimera_", seq_along(chimeric_sequences))

# Combine headers and sequences into a FASTA format
fasta_lines <- Map(function(header, sequence) paste(header, sequence, sep = "\n"), chimera_headers, chimeric_sequences)
