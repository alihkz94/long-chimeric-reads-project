...
def process_blast_record(record, sequences):
    ...
    return {
        ...
        'Sequence': sequences.get(record['qseqid'], '').replace('\n', '').replace('\r', '')
    }

# Function to generate FASTA file from non-chimeric sequences
def generate_fasta(processed_data, output_path):
    non_chimeric_records = [SeqRecord(Seq(row['Sequence']), id=row['SeqID'], description="") 
                            for index, row in processed_data.iterrows() 
                            if row['IsChimeric'] == False]
    SeqIO.write(non_chimeric_records, output_path, "fasta")
...
def main():
    ...
    sequences = {record.id: str(record.seq).replace('\n', '').replace('\r', '') for record in SeqIO.parse(args.input_fasta, "fasta")}
    ...
    generate_fasta(processed_data, "rescued_chimeras.fasta")
